//
//  CustomUITableViewCellPedidoViewController.m
//  GoChef!
//
...
@implementation CustomUITableViewCellPedidoViewController
...
-(void) setContentWith:(OrderFoodClass *)newOFC_food {
    ...
    // Comprobamos que no sea una oferta -> ocultamos la flecha
    if ((_OFC_food.B_is_offer) || (_OFC_food.B_is_price_offer)) [UIIV_flecha setAlpha:0.0f];
...
//# ------------------------------------------------------------------------------------------------------------
//#	Procedimiento   : UIB_cell_TouchUpInside:bundle:
...
-(IBAction) UIB_cell_TouchUpInside:(id)sender {
    ...
    // Comprobamos que no sea una oferta
    if ((_OFC_food.B_is_offer) || (_OFC_food.B_is_price_offer) || (_OFC_food.B_is_offer_facebook))
        return;
    
    // Indicamos al delegado que se ha pulsado sobre la celda
    if (_delegate != nil) [_delegate cellTouched:_OFC_food];
}
