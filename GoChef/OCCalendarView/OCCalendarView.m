...
@synthesize offerDays = _offerDays;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    SingletonGlobal *globalVar = [SingletonGlobal sharedGlobal];
    ...
    if(CGRectContainsPoint(CGRectMake(55, 40, 30, 35), point)) {
        ...
        NSString *cDate = [NSString stringWithFormat:@"%d-%02d",currentYear,currentMonth];        
        [self setOfferDays: [globalVar.JPC_json UMNI_getDatesOffers:globalVar.OC_order.RC_restaurant.NSI_idrestaurant inMonth:cDate]];
        [daysView setOffers:self.offerDays];
...
-(void) renderCalendarViewForPhone
{
    ...
    NSArray *dayTitlesTmp = [dateFormatter shortStandaloneWeekdaySymbols];
    NSMutableArray *dayTitles = [[NSMutableArray alloc] initWithCapacity:7];
    ...
    for (int i = 0; i <7; i++) {
        if (i == 0) {
            [dayTitles setObject:[dayTitlesTmp objectAtIndex:i] atIndexedSubscript:6];
        } else {
            [dayTitles setObject:[dayTitlesTmp objectAtIndex:i] atIndexedSubscript:i-1];
        }
    }
    ...
    UIColor* darkColor = [UIColor clearColor];
    UIColor* boxStroke = [UIColor clearColor];
