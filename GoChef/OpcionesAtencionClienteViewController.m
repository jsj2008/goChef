//
//  OpcionesAtencionClienteViewController.m
//  GoChef!
//
...
@implementation OpcionesAtencionClienteViewController
...
-(void) viewWillAppear:(BOOL)animated {
	...
	self.navigationItem.title = @"Atención al Cliente";
...
-(void) keyboardDidShow:(NSNotification *) notification {
    ...
    // Posicionamos UIVew
    [UIV_contenido setFrame:CGRectMake(0.0f, -110.0f, UIV_contenido.frame.size.width, UIV_contenido.frame.size.height)];
...
- (void)animationDidStop:(NSString*)animationID finished:(BOOL)finished context:(void *)context {
    ...
    if (_B_enviar) {
        ...
        [globalVar.MC_mensaje setTMT_type:TMT_problema];
...
-(void) setsuggestionsproblemsSuccessful:(NSNotification *)notification {
    ...
    [globalVar showAlerMsgWith:@"Información"
                       message:@"El mensaje ha sido enviado correctamente. Te contestaremos por email lo antes posible."];
}
...
//# ------------------------------------------------------------------------------------------------------------
//#	Procedimiento   : textViewDidBeginEditing
...
-(void) textViewDidEndEditing:(UITextView *)textView {
    ...
    // Comprobamos si se ha dejado vacío
    if ([textView.text length] == 0) {
        
        // Volvemos a copiar el texto explicativo
        [textView setText:@"Descríbenos tu problema.\nSólo necesitamos suficiente información para saber como ayudarte (sabemos lo difícil que puede llegar a ser escribir con el móvil)"];
